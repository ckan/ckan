...
import ckan
from ckan.plugins import SingletonPlugin, implements, IPackageController, toolkit
...
def translate_data_dict(data_dict):
    ...
    fallback_lang_code = config.safe('ckan.locale_default')
...
class MultilingualDataset(SingletonPlugin):
    ...
    LANGS = toolkit.aslist(config.safe('ckan.locale_order')) or ["en"]
    ...
    def before_search(self, search_params):
        ...
        try:
            ...
            if err.message == ('No object (name: request) has been registered '
                ...
                current_lang = config.safe('ckan.locale_default')
            else:
                raise
        except KeyError:
            current_lang = config.safe('ckan.locale_default')
