...
class Activity(domain_object.DomainObject, Base):  # type: ignore
    ...
    def activity_stream_item(
            cls, pkg: model.Package, activity_type: str,
            user_id: str) -> Optional["Activity"]:
        import ckan.model
        import ckan.logic
        ...
        # 'deleted' activity.
        if activity_type == 'changed' and pkg.state == u'deleted':
            if meta.Session.query(cls).filter_by(
                    object_id=pkg.id, activity_type='deleted').all():
...
def activity_dictize(
        activity: Activity, context: Context) -> dict[str, Any]:
...
def _group_activity_query(
    ...
    q: QActivity = model.Session.query(
        ...
        model.Member,
        Activity.object_id == model.Member.table_id
    ).outerjoin(
        model.Package,
        and_(
...
def _filter_activitites_from_users(q: QActivity) -> QActivity:
    ...
    if users_to_avoid:
        # type_ignore_reason: incomplete SQLAlchemy types
        q = q.filter(Activity.user_id.notin_(  # type: ignore
            users_to_avoid))
...
def _filter_activitites_from_type(
    ...
    if include:
        q = q.filter(
            Activity.activity_type.in_(types)  # type: ignore
        )
    else:
        q = q.filter(
            Activity.activity_type.notin_(types)  # type: ignore
        )
    return q
