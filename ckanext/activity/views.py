...
def package_changes(id: str) -> Union[Response, str]:  # noqa
    ...
    """
    activity_id = id
    context = cast(Context, {"auth_user_obj": tk.current_user})
    try:
...
def dashboard() -> str:
    ...
    data_dict: dict[str, Any] = {"user_obj": tk.current_user}
    extra_vars = _extra_template_variables(context, data_dict)
    ...
    extra_vars["followee_list"] = tk.get_action("followee_list")(
        context, {"id": tk.current_user.id, "q": q}
    )
