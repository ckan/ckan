...
def send_email_notifications(
    ...
    if not tk.config.get_value('ckan.activity_streams_email_notifications'):
        raise tk.ValidationError({
            'message': 'ckan.activity_streams_email_notifications'
...
def dashboard_new_activities_count(
    ...
    activities = tk.get_action('dashboard_activity_list')(
        context, data_dict)
...
def activity_show(context: Context,
    ...
    tk.check_access(u'activity_show', context, data_dict)
...
def activity_data_show(
    ...
    tk.check_access(u'activity_data_show', context, data_dict)
...
def activity_diff(context: Context,
    ...
    tk.check_access(u'activity_diff', context, data_dict)

    activity = model.Session.query(model_activity.Activity).get(activity_id)
    if activity is None:
        raise tk.ObjectNotFound()
    prev_activity = model.Session.query(model_activity.Activity) \
    ...
    activities = [model_activity.activity_dictize(activity_obj, context)
                  for activity_obj in activity_objs]
