...
return actualQuery;};my.query=function(queryObj,dataset){if(dataset.endpoint){var wrapper=my.DataStore(dataset.endpoint);}else{var out=my._parseCkanResourceUrl(dataset.url);dataset.id=out.resource_id;var wrapper=my.DataStore(out.endpoint);}
...
if(self.state.attributes.graphType!=='bars'&&label.length>8){label=label.slice(0,5)+"...";}
return label;};var xaxis={};xaxis.tickFormatter=tickFormatter;if(numPoints){var ticks=[],maxTicks=10,x=1,i=0;if(self.state.attributes.graphType!=='bars'){while(x<=maxTicks){if((numPoints/x)<=maxTicks){break;}
x=x+1;}}
for(i=0;i<numPoints;i=i+x){ticks.push(i);}
xaxis.ticks=ticks;}
...
var yfield=self.model.fields.get(field);var y=doc.getFieldValue(yfield);if(self.state.attributes.graphType=='bars'){points.push([y,x]);}else{points.push([x,y]);}});series.push({data:points,label:field,hoverable:true});});return series;}});my.FlotControls=Backbone.View.extend({className:"editor",template:' \
  <div class="editor"> \
