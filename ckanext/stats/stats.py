...
class Stats(object):
    ...
    def largest_groups(cls, limit=10):
        ...
        j = join(activity, package, activity.c.object_id ==
                 package.c.id)

        s = select(
            [package.c.owner_org,
             func.count(package.c.id)]
        ).select_from(j).group_by(package.c.owner_org).where(
            and_(
                package.c.owner_org != None,
                activity.c.activity_type == 'new package',
                package.c.private == False, package.c.state == 'active',
            )
        ).order_by(func.count(package.c.id).desc()).limit(limit)
    ...
    def most_edited_packages(cls, limit=10):
        ...
        s = select([package.c.id, func.count(activity.c.id)],
                   from_obj=[activity.join(
                       package, activity.c.object_id == package.c.id)]).\
            where(and_(package.c.private == False,
                       activity.c.activity_type == 'changed package',
                       package.c.state == 'active')).\
            group_by(package.c.id).\
            order_by(func.count(activity.c.id).desc()).limit(limit)
        res_ids = model.Session.execute(s).fetchall()
        res_pkgs = [(model.Session.query(model.Package).get(
            text_type(pkg_id)), val) for pkg_id, val in res_ids]
        return res_pkgs
    ...
    def get_package_revisions(cls):
        ...
        package = table('package')
        activity = table('activity')
        s = select([package.c.id, activity.c.timestamp],
                   from_obj=[activity.join(
                       package, activity.c.object_id == package.c.id)]).\
    ...
    def get_by_week(cls, object_type):
        ...
        def objects_by_week():
            ...
            for pkg_id, date_field in objects:
                ...
                if date_ >= week_ends:
                    weekly_pkg_ids.append(build_weekly_stats(
                        week_commences, pkg_id_stack))
                    pkg_id_stack = []
    ...
    def get_new_packages(cls):
        ...
        def new_packages():
            ...
            package = table('package')
            activity = table('activity')
            s = select([package.c.id, func.min(activity.c.timestamp)],
                       from_obj=[activity.join(
                           package, activity.c.object_id == package.c.id)]).\
                group_by(package.c.id).order_by(func.min(activity.c.timestamp))
            res = model.Session.execute(s).fetchall()  # [(id, datetime), ...]
            res_pickleable = []
    ...
    def get_num_packages_by_week(cls):
        def num_packages():
            ...
            first_date = (min(datetime.datetime.strptime(
                new_packages_by_week[0][0], DATE_FORMAT),
                datetime.datetime.strptime(deleted_packages_by_week[0][0],
                                           DATE_FORMAT))).date()
            cls._cumulative_num_pkgs = 0
            new_pkgs = []
            deleted_pkgs = []
            ...
            while week_ends <= today:
                ...
                if datetime.datetime.strptime(
                    ...
                    new_pkg_ids = new_packages_by_week[
