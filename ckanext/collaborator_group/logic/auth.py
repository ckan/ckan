...
from ckan.types import AuthFunction, Context, DataDict, AuthResult
...
def package_collaborator_create_group(context: Context,
...
def package_collaborator_delete_group(context: Context,
                                      data_dict: DataDict) -> AuthResult:
...
def package_collaborator_list_for_group(context: Context, data_dict: DataDict) -> Union["dict[str, bool]", bool]:
    user = context['user']
    ...
    if is_collaborator: return {"success": True}
...
def package_update(next_auth: AuthFunction, context: Context, data_dict: DataDict) -> Union[bool, AuthResult]:
...
def group_is_collaborator_on_dataset(
    user_obj: "model.User", dataset_id: str,
    capacity: "Optional[Union[str, list[str]]]" = None) -> Union[bool, "dict[str, bool]"]:
    '''
