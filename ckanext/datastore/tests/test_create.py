...
class TestDatastoreCreateNewTests:
    ...
    def test_create_exceeds_column_name_limit(self):
        ...
        data = {
            ...
            'fields': [{
                'id':
                'This is a really long name for a column. Column names '
...
class TestDatastoreCreate:
    ...
    def test_create_invalid_field_name(self, app):
        ...
        auth = {'Authorization': str(self.sysadmin_user.apikey)}
        invalid_names = [
            '_author', '"author', '', ' author', 'author ', '\tauthor',
            'author\n'
        ]
    ...
    def test_create_basic(self, app):
        ...
        auth = {'Authorization': str(self.sysadmin_user.apikey)}
        res = app.post('/api/action/resource_show',
                       params=postparams,
                       extra_environ=auth)
        res_dict = json.loads(res.body)
        ...
        for i, row in enumerate(results):
            ...
            assert data['records'][i].get('author') == (json.loads(
        ...
        for alias in aliases:
            ...
            results = [
                row for row in c.execute(u'select * from "{0}"'.format(
                    resource.id))
            ]
            results_alias = [
        ...
        for i, row in enumerate(results):
            ...
            assert all_data[i].get('author') == (json.loads(row['author'][0])
                                                 if row['author'] else None)
        ...
        for i, row in enumerate(results):
            assert all_data[i].get('boo%k') == row['boo%k'], (
                i, all_data[i].get('boo%k'), row['boo%k'])
            assert all_data[i].get('author') == (json.loads(row['author'][0])
                                                 if row['author'] else None)

        results = c.execute(
            '''select * from "{0}" where _full_text @@ to_tsquery('dostoevsky') '''
            .format(resource.id))
        self.Session.remove()
        assert results.rowcount == 2
    ...
    def test_guess_types(self, app):
        ...
        postparams = '%s=1' % json.dumps(data)
        auth = {'Authorization': str(self.sysadmin_user.apikey)}
        res = app.post('/api/action/datastore_delete',
                       params=postparams,
                       extra_environ=auth,
        ...
        assert types == [
            u'int4', u'tsvector', u'nested', u'int4', u'text', u'timestamp',
            u'float8'
        ], types
...
class TestDatastoreCreateTriggers:
    ...
    def test_create_trigger_applies_to_records(self, app):
        ...
        with app.flask_app.test_request_context():
            res = helpers.call_action(u'datastore_create',
                                      resource={u'package_id': ds['id']},
                                      fields=[{
                                          u'id': u'spam',
                                          u'type': u'text'
                                      ...
                                      triggers=[{
                                          u'function': u'spamify_trigger'
                                      }])
        assert_equal(
            helpers.call_action(u'datastore_search',
                                fields=[u'spam'],
    ...
    def test_upsert_trigger_applies_to_records(self, app):
        ...
        with app.flask_app.test_request_context():
            ...
            helpers.call_action(u'datastore_upsert',
                                method=u'insert',
