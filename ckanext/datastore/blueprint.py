...
def dump(resource_id: str):
    ...
    public_table_search = config.get('ckan.datastore.public_table_search')
    if resource_id not in public_table_search:
...
class DictionaryView(MethodView):
    ...
    def _prepare(self, id: str, resource_id: str) -> dict[str, Any]:
        try:
            check_access("datastore_create", {}, {"resource_id": resource_id})
