...
function load_datatable(CKAN_MODULE){
  ...
  const ellipsesLength = CKAN_MODULE.options.ellipsisLength;
  ...
  const defaultSortOrder = [[1, "asc"]];  // _id col
  ...
  function cell_renderer(_data, _type, _row, _meta, _dictionary_field){
    if( _type == 'display' ){
      ...
      _data = DataTable.render.ellipsis(ellipsesLength, _meta.row, _dictionary_field.id, false)(_data, _type, _row, _meta);
      return _data;
    }
    return _data;
  }
