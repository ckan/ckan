describe('ckan.i18n', function () {
  ...
  describe('.ngettext(singular, plural, number, [values])', function () {
    beforeEach(() => {
      cy.window().then(win => {
        Cypress.env('ngettext', win.ckan.i18n.ngettext);
      })
    })

    it('should return the translated strings', function () {
      assert.equal(Cypress.env('ngettext')('bar', 'bars', 1), 'BAR');
      assert.equal(Cypress.env('ngettext')('bar', 'bars', 0), 'BARS');
      assert.equal(Cypress.env('ngettext')('bar', 'bars', 2), 'BARS');
    });

    it('should return the key when no translation exists', function () {
      assert.equal(
        Cypress.env('ngettext')('no translation', 'no translations', 1),
        'no translation'
      );
      assert.equal(
        Cypress.env('ngettext')('no translation', 'no translations', 0),
        'no translations'
      );
      assert.equal(
        Cypress.env('ngettext')('no translation', 'no translations', 2),
        'no translations'
      );
    });
    ...
    it('should fill in placeholders', function () {
      assert.equal(
        Cypress.env('ngettext')('%(color)s shirt', '%(color)s shirts', 1, {color: 'RED'}),
        'RED SHIRT'
      );
      assert.equal(
        Cypress.env('ngettext')('%(color)s shirt', '%(color)s shirts', 0, {color: 'RED'}),
        'RED SHIRTS'
      );
      assert.equal(
        Cypress.env('ngettext')('%(color)s shirt', '%(color)s shirts', 2, {color: 'RED'}),
        'RED SHIRTS'
      );
    });
    ...
    it('should fill in placeholders when no translation exists', function () {
      assert.equal(
        Cypress.env('ngettext')('no %(attr)s translation', 'no %(attr)s translations',
                 1, {attr: 'good'}),
        'no good translation'
      );
      assert.equal(
    ...
    it('should provide a magic `num` placeholder', function () {
      assert.equal(Cypress.env('ngettext')('%(num)d item', '%(num)d items', 1), '1 ITEM');
      assert.equal(Cypress.env('ngettext')('%(num)d item', '%(num)d items', 0), '0 ITEMS');
      assert.equal(Cypress.env('ngettext')('%(num)d item', '%(num)d items', 2), '2 ITEMS');
    });

    it('should provide `num` when no translation exists', function () {
      assert.equal(
        Cypress.env('ngettext')('%(num)d missing translation',
