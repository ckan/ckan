...
`SCSS <https://sass-lang.com/>`_ (this depends on
...
.. note:: If you use the package on the default Ubuntu repositories (eg ``sudo apt-get install nodejs``),
    the node binary will be called ``nodejs``. This might prevent the CKAN SCSS script to
...
We use ``gulp`` to make our SCSS compiler a watcher
style script.
...
css
    ...
    production builds generated by SCSS.
scss
...
Because all the stylesheets are using SCSS we need to compile them
...
There are many SCSS files which attempt to group the styles in useful
