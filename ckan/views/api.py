...
def _get_request_data(try_url_params=False):
    ...
    if request.method == 'PUT' and not request_data:
        raise ValueError('Invalid request. Please use the POST method for '
                         'your request')
...
def user_autocomplete(ver=API_REST_DEFAULT_VERSION):
    ...
    if q:
        ...
        data_dict = {'q': q, 'limit': limit, 'ignore_self': ignore_self}
...
def group_autocomplete(ver=API_REST_DEFAULT_VERSION):
    ...
    if q:
        ...
        data_dict = {'q': q, 'limit': limit}
        group_list = get_action('group_autocomplete')(context, data_dict)
...
def organization_autocomplete(ver=API_REST_DEFAULT_VERSION):
    ...
    if q:
        ...
        organization_list = get_action(
            'organization_autocomplete')(context, data_dict)
...
def i18n_js_translations(lang, ver=API_REST_DEFAULT_VERSION):
    ...
    if not os.path.exists(source):
        return '{}'
    translations = json.load(open(source, 'r'))
...
api.add_url_rule('/<int(min=1, max={0}):ver>'.format(API_MAX_VERSION),
