...
return path;},getTemplate:function(filename,params,success,error){var url=this.url('/api/1/util/snippet/'+encodeURIComponent(filename));if(typeof params==='function'){error=success;success=params;params={};}
...
var map={};var raw=data.ResultSet&&data.ResultSet.Result||{};var items=jQuery.map(raw,function(item){item=typeof item==='string'?item:item.Name||item.Format||'';item=jQuery.trim(item);var lowercased=item.toLowerCase();var returnObject=options&&options.objects===true;if(lowercased&&!map[lowercased]){map[lowercased]=1;return returnObject?{id:item,text:item}:item;}
