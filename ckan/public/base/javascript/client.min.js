...
var map={};var raw=jQuery.isArray(data)?data:data.ResultSet&&data.ResultSet.Result||{};var items=jQuery.map(raw,function(item){var key=typeof options.key!='undefined'?item[options.key]:false;var label=typeof options.label!='undefined'?item[options.label]:false;let children=item.children;item=typeof item==='string'?item:item.name||item.Name||item.Format||'';item=jQuery.trim(item);key=key?key:item;label=label?label:item;let ret={text:label};if(children===undefined){ret.id=key;}else{for(i=0,l=children.length;i<l;i=i+1){children[i].id=children[i][options.key];children[i].text=children[i][options.label];}
