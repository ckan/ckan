...
module.registry={};module.instances={};module.initialize=function(){var registry=module.registry;ckan.pubsub.enqueue();jQuery('[data-module]',document.body).each(function(index,element){var names=jQuery.trim(this.getAttribute(MODULE_PREFIX)).split(' ');jQuery.each(names,function(index,name){var Module=registry[name];if(Module&&typeof Module==='function'){module.createInstance(Module,element);}});});ckan.pubsub.dequeue();return module;};module.createInstance=function(Module,element){var options=module.extractOptions(element);var sandbox=ckan.sandbox(element,options);var instance=new Module(element,options,sandbox);if(typeof instance.initialize==='function'){instance.initialize();}
