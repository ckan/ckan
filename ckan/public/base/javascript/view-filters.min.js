...
var fieldsStr=$.map(fields,function(field){return filter.replace(/(?<!#):/g,'#:')+':'+field.replace(/(?<!#)\|/g,'#|').replace(/(?<!#):/g,'#:');});return fieldsStr.join('|');}).join('|');}
return $.param(params);}
...
return arrayCopy;}
function _initialize(queryString){var searchParams=Qs.parse(queryString,{ignoreQueryPrefix:true});if(searchParams.filters){var filters={},fieldValuesStr=String(searchParams.filters).split(/(?<!#)\|/g),i,len;for(i=0,len=fieldValuesStr.length;i<len;i++){var fieldValue=fieldValuesStr[i].replace(/#:/g,':').match(/([^:]+):(.*)/),field=fieldValue[1],value=fieldValue[2];filters[field]=filters[field]||[];filters[field].push(value);}
