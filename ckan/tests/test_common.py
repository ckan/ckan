...
def test_del_works():
    my_conf = CKANConfig()
    my_conf["test_key_1"] = "Test value 1"
    del my_conf["test_key_1"]
    assert "test_key_1" not in my_conf


def test_get_item_works():
    my_conf = CKANConfig()
    my_conf["test_key_1"] = "Test value 1"
    assert my_conf.get("test_key_1") == "Test value 1"
...
def test_repr_works():
    ...
    my_conf["test_key_1"] = "Test value 1"
    if six.PY3:
        assert repr(my_conf) == "{'test_key_1': 'Test value 1'}"
    else:
        assert repr(my_conf) == "{'test_key_1': 'Test value 1'}"
...
def test_len_works():
    my_conf = CKANConfig()
    my_conf["test_key_1"] = "Test value 1"
    my_conf["test_key_2"] = "Test value 2"
...
def test_keys_works():
    ...
    my_conf["test_key_1"] = "Test value 1"
    my_conf["test_key_2"] = "Test value 2"
    assert sorted(my_conf.keys()) == ["test_key_1", "test_key_2"]
...
def test_for_in_works():
    ...
    for key in my_conf:
        cnt += 1
        assert key.startswith("test_key_")
    assert cnt == 2
...
def test_iteritems_works():
    ...
    for key, value in six.iteritems(my_conf):
        cnt += 1
        assert key.startswith("test_key_")
        assert value.startswith("Test value")
...
def test_setting_a_key_sets_it_on_flask_config_if_app_context(
    ...
    with app.flask_app.app_context():
        monkeypatch.setitem(ckan_config, "ckan.site_title", "Example title")
...
def test_deleting_a_key_deletes_it_on_ckan_config():
    del ckan_config["ckan.site_title"]
    assert "ckan.site_title" not in ckan_config
...
def test_deleting_a_key_delets_it_on_flask_config(
    ...
    with app.flask_app.app_context():
        monkeypatch.setitem(ckan_config, "ckan.site_title", "Example title")
        del ckan_config["ckan.site_title"]
        assert "ckan.site_title" not in flask.current_app.config
...
def test_update_works_on_flask_config(app):
    with app.flask_app.app_context():
        ckan_config["ckan.site_title"] = "Example title"

        ckan_config.update(
            {"ckan.site_title": "Example title 2", "ckan.new_key": "test"}
        )
        assert (
            flask.current_app.config["ckan.site_title"] == "Example title 2"
        )
        assert flask.current_app.config["ckan.new_key"] == "test"


def test_config_option_update_action_works_on_flask(app, reset_db, ckan_config):
    params = {"ckan.site_title": "Example title action"}
    helpers.call_action("config_option_update", {}, **params)
    assert ckan_config["ckan.site_title"] == "Example title action"
    reset_db()


def test_params_also_works_on_flask_request(app):
    with app.flask_app.test_request_context("/?a=1"):
        assert "a" in ckan_request.args
        assert "a" in ckan_request.params


def test_other_missing_attributes_raise_attributeerror_exceptions(app):
    with app.flask_app.test_request_context("/?a=1"):
        with pytest.raises(AttributeError):
            getattr(ckan_request, "not_here")


def test_flask_g_is_used_on_a_flask_request(app):
    with app.flask_app.test_request_context():
        assert "flask.g" in text_type(ckan_g)
        flask.g.user = "example"
        assert ckan_g.user == "example"


def test_can_also_use_c_on_a_flask_request(app):
    with app.flask_app.test_request_context():
        flask.g.user = "example"
        assert ckan_c.user == "example"

        ckan_g.user = "example2"
        assert ckan_c.user == "example2"
