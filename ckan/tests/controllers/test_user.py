...
def test_change_user(app):
    ...
    response = app.get(url)
    assert (
        '<a href="/user/{}">Mr. Changed Name'.format(user["name"]) in response
    )
...
def test_create_dataset(app):
    ...
    assert (
        '<a href="/dataset/{}">Test Dataset'.format(dataset["id"]) in response
    )
...
def test_delete_group_by_updating_state(app):
    ...
    assert '<a href="/user/{}">Mr. Test User'.format(user["name"]) in response
    assert "deleted the group" in response
    assert '<a href="/group/{}">Test Group'.format(group["name"]) in response
