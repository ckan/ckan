...
class TestPublisherGroups(FunctionalTestCase):
    ...
    def _run_success_test(self, username, action):
        ...
        try:
            ...
            assert False, "The user should have %s access: %r." %\
                (action, e.extra_msg)
...
class TestPublisherPackageRelationships(FunctionalTestCase):
    ...
    def test_delete_success(self):
        ...
        try:
            self.auth.check_access('package_relationship_delete', context,
                                   {'id': p1.id, 'id2': p2.id})
        except NotAuthorized, e:
            assert False, "The user should have %s access: %r." %\
                ('package_relationship_delete', e.extra_msg)
