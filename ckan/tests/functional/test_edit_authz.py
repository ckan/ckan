...
class TestEditAuthz(TestController):
    ...
    def test_access_to_authz(self):
        ...
        for (c,i) in [('package', self.pkg),('group', self.group)]:
    ...
    def roles_list(self, authzobj):
        ...
        return [ (r.user.name, r.role) for r in authzobj.roles if r.user]
    ...
    def change_roles(self, user):
        ...
        for (c,i,var,const1) in [('package', self.pkg, self.package_roles, self.group_roles),\
                        ('group', self.group, self.group_roles, self.package_roles)]:
    ...
    # do the change roles both as package/group admin, and also as sysadmin.
