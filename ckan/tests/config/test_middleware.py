...
class MockRoutingPlugin(p.SingletonPlugin):
    ...
    def before_map(self, _map):
        ...
        _map.connect(u'/from_pylons_extension_before_map',
                     controller=self.controller,
                     action=u'view')

        _map.connect(u'/from_pylons_extension_before_map_post_only',
                     controller=self.controller,
                     action=u'view',
                     conditions={u'method': u'POST'})
        # This one conflicts with an extension Flask route
        _map.connect(u'/pylons_and_flask',
                     controller=self.controller,
                     action=u'view')
        ...
        _map.connect(u'/pylons_route_flask_url_for',
                     controller=self.controller,
                     action=u'test_flask_url_for')
        _map.connect(u'/pylons_translated',
                     controller=self.controller,
    ...
    def after_map(self, _map):
        ...
        _map.connect(u'/from_pylons_extension_after_map',
                     controller=self.controller,
                     action=u'view')
    ...
    def get_blueprint(self):
        ...
        blueprint.add_url_rule(u'/simple_flask', u'flask_plugin_view',
                               flask_plugin_view)

        blueprint.add_url_rule(u'/flask_route_pylons_url_for',
                               u'flask_route_pylons_url_for',
                               flask_plugin_view_url_for)
        blueprint.add_url_rule(u'/flask_translated', u'flask_translated',
...
class TestAppDispatcher:
    ...
    def test_ask_around_pylons_core_route_get(self, app):
        ...
        assert answers == [(False, u'flask_app'),
                           (True, u'pylons_app', u'core')]

    def test_ask_around_pylons_core_route_post(self, app):
        environ = {
            u'PATH_INFO': u'/tag',
    ...
    @pytest.mark.ckan_config(u'ckan.plugins', u'test_routing_plugin')
    def test_ask_around_pylons_extension_route_get_before_map(self, app):
        environ = {
    ...
    @pytest.mark.ckan_config(u'ckan.plugins', u'test_routing_plugin')
    def test_ask_around_pylons_extension_route_post(self, app):
        environ = {
    ...
    @pytest.mark.ckan_config(u'ckan.plugins', u'test_routing_plugin')
    def test_ask_around_pylons_extension_route_post_using_get(self, app):
        environ = {
            u'PATH_INFO': u'/from_pylons_extension_before_map_post_only',
            u'REQUEST_METHOD': u'GET',
        }
    ...
    @pytest.mark.ckan_config(u'ckan.plugins', u'test_routing_plugin')
    def test_ask_around_pylons_extension_route_get_after_map(self, app):
        environ = {
    ...
    def test_flask_core_route_is_served_by_flask(self, app):
        res = app.get(u'/')

        assert res.environ[u'ckan.app'] == u'flask_app'

    @pytest.mark.ckan_config(u'ckan.plugins', u'test_routing_plugin')
    def test_flask_extension_route_is_served_by_flask(self, app):
        res = app.get(u'/simple_flask')
        assert res.environ[u'ckan.app'] == u'flask_app'

    @pytest.mark.ckan_config(u'ckan.plugins', u'test_routing_plugin')
    def test_pylons_extension_route_is_served_by_pylons(self, app):

        res = app.get(u'/from_pylons_extension_before_map')

        assert res.environ[u'ckan.app'] == u'pylons_app'
        assert res.body == u'Hello World, this is served from a Pylons extension'

    def test_flask_core_and_pylons_core_route_is_served_by_flask(self, app):
        '''
        This should never happen in core, but just in case
        '''
        res = app.get(u'/flask_core')
...
class TestPylonsUserIdentifiedInRequest:
    ...
    def test_user_objects_in_c_normal_user(self, app):
        ...
        assert resp.tmpl_context.user == user[u'name']
        assert resp.tmpl_context.userobj == test_user_obj
        assert resp.tmpl_context.author == user[u'name']
    ...
    def test_user_objects_in_c_anon_user(self, app):
        ...
        resp = app.get(u'/from_pylons_extension_before_map',
                       extra_environ={u'REMOTE_USER': str(u'')})
