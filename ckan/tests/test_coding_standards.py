...
#
# The following files have known PEP8 errors.  When the files get to a point
# of not having any such errors they should be removed from this list to
# prevent new errors being added to the file.
...
#
# The following files contain one or more `from ... import *` lines which
# should not be used in ckan where possible.  If the files get fixed they
...
# external functions.  When files are fixed they should be removed from this
# list.
#
...
# Nasty str() issues
#
# There are places in ckan where code is like `'...%s..' % str(..)` these
# cause problems when unicode is present but can remain dormant for a long
# time before the issue is apparent so try to remove these.  The value is
# converted to a string anyway so the str() is unneeded in any place.
...
NASTY_STR_BLACKLIST_FILES = [
    ...
    'ckan/logic/auth/get.py',
    'ckan/logic/validators.py',
    'ckan/tests/coding_standards.py',  # example causes error
    'ckan/tests/functional/api/test_revision_search.py',
    'ckan/tests/functional/test_pagination.py',
    'ckan/tests/models/test_package_relationships.py',
    'ckanext/resourceproxy/controller.py',
...
class TestNastyString(object):
    ...
    @classmethod
    def process(cls):
        blacklist = NASTY_STR_BLACKLIST_FILES
...
class TestImportFromCkan(object):
    ...
    @classmethod
    def process(cls):
        blacklist = CKAN_IMPORTS_BLACKLIST_FILES
...
class TestPep8(object):
    ...
    @classmethod
    def process(cls):
        blacklist = PEP8_BLACKLIST_FILES
        for path, filename in process_directory(base_path):
