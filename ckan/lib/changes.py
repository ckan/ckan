...
def check_resource_changes(change_list, old, new, old_activity_id):
    ...
    # to extra fields
    fields = [
        'package_id', 'url', 'revision_id', 'description',
        'format', 'hash', 'name', 'resource_type',
        'mimetype', 'mimetype_inner', 'cache_url',
        'size', 'created', 'last_modified', 'metadata_modified',
        'cache_last_updated', 'upload', 'position'
    ]
    ...
    for resource_id in new_resources:
        change_list.append({'type': 'new_resource',
                            'pkg_id': new['id'],
                            'title': new.get('title'),
                            'resource_name':
    ...
    for resource_id in deleted_resources:
        change_list.append({'type': 'delete_resource',
                            'pkg_id': new['id'],
                            'title': new.get('title'),
                            'resource_id': resource_id,
                            'resource_name':
    ...
    for resource_id in resources:
        ...
        if old_metadata.get('name') != new_metadata.get('name'):
            change_list.append({'type': 'resource_name',
                                'title': new.get('title'),
                                'old_pkg_id': old['id'],
                                'new_pkg_id': new['id'],
                                'resource_id': resource_id,
        ...
        if not old_metadata.get('format') and new_metadata.get('format'):
            change_list.append({'type': 'resource_format',
                                'method': 'add',
                                'pkg_id': new['id'],
                                'title': new.get('title'),
                                'resource_id': resource_id,
                                'resource_name':
                                new_resource_dict[resource_id].get('name'),
                                'org_id': new.get('organization')['id']
                                    if new.get('organization') else '',
        ...
        if not old_metadata.get('description') and \
            ...
            change_list.append({'type': 'resource_desc',
                                'method': 'add',
                                'pkg_id': new['id'],
                                'title': new.get('title'),
                                'resource_id': resource_id,
        ...
        if old_metadata.get('url') != new_metadata.get('url'):
            change_list.append({'type': 'new_file',
                                'pkg_id': new['id'],
                                'title': new.get('title'),
                                'resource_id': resource_id,
                                'resource_name':
        ...
        if len(new_fields) == 1:
            if new_metadata[new_fields[0]]:
                change_list.append({'type': 'resource_extras',
...
def check_metadata_changes(change_list, old, new):
    ...
    if old.get('private') != new.get('private'):
        change_list.append({'type': 'private', 'pkg_id': new.get('id'),
                            'title': new.get('title'),
                            'new':
                            'Private' if bool(new.get('private'))
                            else 'Public'})
...
def _title_change(change_list, old, new):
    ...
    '''
    change_list.append({'type': 'title', 'id': new.get('name'),
...
def _org_change(change_list, old, new):
    ...
    if old.get('owner_org') and new.get('owner_org'):
        change_list.append({'type': 'org',
                            'method': 'change',
                            'pkg_id': new.get('id'),
                            'title': new.get('title'),
                            'old_org_id': old.get('organization').get('id'),
                            'old_org_title':
                            old.get('organization').get('title'),
                            'new_org_id': new.get('organization').get('id'),
                            'new_org_title':
...
def _maintainer_change(change_list, old, new):
    ...
    if old.get('maintainer') and new.get('maintainer'):
        change_list.append({'type': 'maintainer', 'method': 'change',
                            'pkg_id': new.get('id'),
                            'title': new.get('title'), 'new_maintainer':
                            new['maintainer'], 'old_maintainer':
                            old['maintainer']})
    # if they removed the maintainer
    elif not new.get('maintainer'):
        change_list.append({'type': 'maintainer', 'pkg_id':
                            new.get('id'), 'title': new.get('title'),
                            'method': 'remove'})
...
def _maintainer_email_change(change_list, old, new):
    ...
    if old.get('maintainer_email') and new.get('maintainer_email'):
        change_list.append({'type': 'maintainer_email', 'pkg_id':
                            new.get('id'), 'title': new.get('title'),
                            'new_maintainer_email':
                                new.get('maintainer_email'),
                            'old_maintainer_email':
                            old.get('maintainer_email'),
                            'method': 'change'})
...
def _author_change(change_list, old, new):
    ...
    if old.get('author') and new.get('author'):
        change_list.append({'type': 'author', 'pkg_id': new.get('id'),
                            'title': new.get('title'), 'new_author':
                            new.get('author'), 'old_author':
                                old.get('author'),
                            'method': 'change'})
    # if they removed the author
    elif not new.get('author'):
        change_list.append({'type': 'author', 'pkg_id': new.get('id'),
                            'title': new.get('title'), 'method': 'remove'})
    # if there wasn't one there before
    else:
        change_list.append({'type': 'author', 'pkg_id': new.get('id'),
                            'title': new.get('title'), 'new_author':
                            new.get('author'), 'method': 'add'})
...
def _author_email_change(change_list, old, new):
    ...
    if old.get('author_email') and new.get('author_email'):
        change_list.append({'type': 'author_email', 'pkg_id':
                            new.get('id'), 'title': new.get('title'),
...
def _notes_change(change_list, old, new):
    ...
    if old.get('notes') and new.get('notes'):
        change_list.append({'type': 'notes', 'pkg_id':
                            new.get('id'), 'title': new.get('title'),
...
def _tag_change(change_list, new_tags, old_tags, new):
    ...
    if len(deleted_tags) == 1:
        change_list.append({'type': 'tags', 'method': 'remove_one',
                            'pkg_id': new.get('id'),
                            'title': new.get('title'),
                            'tag': deleted_tags_list[0]})
    elif len(deleted_tags) > 1:
        change_list.append({'type': 'tags', 'method': 'remove_multiple',
                            'pkg_id': new.get('id'),
                            'title': new.get('title'),
    ...
    if len(added_tags) == 1:
        change_list.append({'type': 'tags', 'method': 'add_one', 'pkg_id':
                            new.get('id'), 'title': new.get('title'),
                            'tag': added_tags_list[0]})
    elif len(added_tags) > 1:
        change_list.append({'type': 'tags', 'method': 'add_multiple',
                            'pkg_id': new.get('id'),
                            'title': new.get('title'),
...
def _license_change(change_list, old, new):
    ...
    change_list.append({'type': 'license', 'pkg_id': new.get('id'),
                        'title': new.get('title'),
                        'old_url': old_license_url,
                        'new_url': new_license_url, 'new_title':
                        new.get('license_title'), 'old_title':
                        old.get('license_title')})
...
def _name_change(change_list, old, new):
    ...
    change_list.
    '''
    change_list.append({'type': 'name', 'pkg_id': new.get('id'),
                        'title': new.get('title'), 'old_name':
                        old.get('name'), 'new_name': new.get('name')})
...
def _url_change(change_list, old, new):
    ...
    if old.get('url') and new.get('url'):
        change_list.append({'type': 'url', 'method': 'change',
                            'pkg_id': new.get('id'), 'title':
                            new.get('title'), 'new_url': new.get('url'),
                            'old_url': old.get('url')})
    # if the user removed the source URL
    elif not new.get('url'):
        change_list.append({'type': 'url', 'method': 'remove',
                            'pkg_id': new.get('id'),
                            'title': new.get('title'),
                            'old_url': old.get('url')})
    # if there wasn't one there before
    else:
        change_list.append({'type': 'url', 'method': 'add',
                            'pkg_id': new.get('id'),
                            'title': new.get('title'),
                            'new_url': new.get('url')})
...
def _version_change(change_list, old, new):
    ...
    if old.get('version') and new.get('version'):
        change_list.append({'type': 'version', 'method': 'change',
                            'pkg_id': new.get('id'), 'title':
                            new.get('title'), 'old_version':
                            old.get('version'), 'new_version':
                            new.get('version')})
    # if the user removed the version number
    elif not new.get('version'):
        change_list.append({'type': 'version', 'method': 'remove',
                            'pkg_id': new.get('id'),
                            'title': new.get('title'),
                            'old_version': old.get('version')})
    # if there wasn't one there before
    else:
        change_list.append({'type': 'version', 'method': 'add',
                            'pkg_id': new.get('id'),
                            'title': new.get('title'),
...
def _extension_fields(change_list, old, new):
    ...
    # user or extension
    fields = [
        'owner_org', 'maintainer', 'maintainer_email',
    ...
    for field in addl_fields_list:
        if old.get(field) != new.get(field):
            change_list.append({'type': 'extension_fields',
                                'pkg_id': new.get('id'),
                                'title': new.get('title'),
                                'key': field,
                                'value': new.get(field)})
        ...
        if 'extras' in old:
            ...
            if len(new_fields) == 1:
                if extra_fields_new[new_fields[0]]:
                    change_list.append({'type': 'extra_fields',
                                        'method': 'add_one_value',
                                        'pkg_id': new.get('id'),
                                        'title': new.get('title'),
                                        'key': new_fields[0],
...
def _description_change(change_list, old, new):
    ...
    if old.get('description') and new.get('description'):
        change_list.append({'type': 'description', 'pkg_id':
                            new.get('id'), 'title': new.get('title'),
...
def _image_url_change(change_list, old, new):
    ...
    if old.get('image_url') and new.get('image_url'):
        change_list.append({'type': 'image_url', 'method': 'change',
                            'pkg_id': new.get('id'), 'title':
                            new.get('title'), 'new_image_url':
                            new.get('image_url'), 'old_image_url':
                            old.get('image_url')})
    # if the user removed the image URL
    elif not new.get('image_url'):
        change_list.append({'type': 'image_url', 'method': 'remove',
                            'pkg_id': new.get('id'),
                            'title': new.get('title'),
                            'old_image_url': old.get('image_url')})
    # if there wasn't one there before
    else:
        change_list.append({'type': 'image_url', 'method': 'add',
                            'pkg_id': new.get('id'),
                            'title': new.get('title'),
