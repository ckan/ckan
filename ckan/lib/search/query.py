...
class PackageSearchQuery(SearchQuery):
    def get_all_entity_ids(self, max_results=1000):
        ...
        """
        query = "*:*"
        fq = "+site_id:\"%s\" " % config.normalized('ckan.site_id')
        fq += "+state:active "
    ...
    def run(self, query, permission_labels=None, **kwargs):
        ...
        fq.append('+site_id:%s' % solr_literal(config.normalized('ckan.site_id')))
        ...
        query['facet.limit'] = query.get('facet.limit', config.normalized('search.facets.limit'))
