...
def _local_url(url_to_amend, **kw):
    ...
    root=''
    no_root_root=''  # the root to strip off the left in case of no_root
    if kw.get('qualified', False):
    ...
    if no_root and no_root_root and url.startswith(no_root_root + no_root_root):
        url = url[len(no_root_root):]
