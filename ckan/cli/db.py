...
from alembic.util.exc import CommandError, AutogenerateDiffsDetected
...
@db.command()
def check():
    """Check if database schema matches definition of models."""
    with _repo_for_plugin("") as repo:
        repo.setup_migration_version_control()
        try:
            alembic_check(repo.alembic_config)

        except AutogenerateDiffsDetected as err:
            error_shout("There are unapplied changes to the database schema. ")
            for diff in err.diffs:
                click.secho(diff[0], bold=True)
                click.echo(f"\t{diff[1:]}")
            raise click.Abort
