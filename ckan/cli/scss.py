...
@click.command(
    name=u'scss',
    short_help=u'Compile all root scss documents into their CSS counterparts')
def scss():
    ...
    custom_scss = os.path.join(root, u'scss', u'_custom.scss')
    for color in _custom_css:
        f = open(custom_scss, u'w')
        f.write(_custom_css[color])
        f.close()
        _compile_scss(root, command, color)
    f = open(custom_scss, u'w')
    f.write(u'// This file is needed in order for `gulp build` to '
            u'compile other color schemes\n')
    f.close()
    _compile_scss(root, command, u'main')


def _compile_scss(root, command, color):
