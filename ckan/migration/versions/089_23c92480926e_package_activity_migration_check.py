...
Once yo've done that, the detailed history is visible in Activity Stream
...
https://github.com/ckan/ckan/wiki/Migrate-package-activity
        ...
        are_any_datasets = bool(
            conn.execute('SELECT id FROM PACKAGE LIMIT 1').rowcount
        )
        # no need to tell the user if there are no datasets - this could just
        # be a fresh CKAN install
        if are_any_datasets:
            print(
                'You have no unmigrated package activities - you do not '
