...
This should be run once yo've upgraded to CKAN 2.9.
...
def num_activities_migratable():
    from ckan import model
    num_activities = model.Session.execute('''
    SELECT count(*) FROM activity a JOIN package p ON a.object_id=p.id
    WHERE a.activity_type IN ('new package', 'changed package')
    AND p.private = false;
    ''').fetchall()[0][0]
...
def migrate_all_datasets():
    ...
    print('  with {} activities'.format(num_activities))
    print_errors(errors)
...
def migrate_dataset(dataset_name, errors):
    ...
    package_activity_stream = logic.get_action('package_activity_list')(
        get_context(), {'id': dataset_name, 'include_hidden_activity': True})
    ...
    for i, activity in enumerate(reversed(package_activity_stream)):
        ...
        #  'id': '62107f87-7de0-4d17-9c30-90cbffc1b296',
        #  'object_id': '7c6314f5-c70b-4911-8519-58dc39a8e340',
        #  'revision_id': 'c3e8670a-f661-40f4-9423-b011c6a3a11d',
        #  'timestamp': '2018-04-20T16:11:45.363097',
        #  'user_id': '724273ac-a5dc-482e-add4-adaf1871f8cb'}
        ...
        if 'resources' in activity_obj.data.get('package', {}):
            print('    activity has full dataset already recorded'
                  ' - no action')
            continue
        ...
        try:
            assert activity_obj.revision_id, \
                'Revision missing on the activity'
            dataset = logic.get_action('package_show')(
                context,
                {'id': activity['object_id'], 'include_tracking': False})
        except Exception as exc:
            if isinstance(exc, logic.NotFound):
                error_msg = 'Revision missing'
            else:
                error_msg = text_type(exc)
            print('    Error: {}! Skipping this version '
    ...
    if model.Session.dirty:
        ...
        print('  saved')
    print('  This package\'s {} activities are migrated'.format(
...
def wipe_activity_detail(delete_activity_detail):
    ...
    if delete_activity_detail is None:
        delete_activity_detail = \
            input('Delete activity_detail table content? (y/n):')
    if delete_activity_detail.lower()[:1] != 'y':
        return
    from ckan import model
    model.Session.execute('DELETE FROM "activity_detail";')
    model.Session.commit()
    print('activity_detail deleted')
